...
@Component({
  ...
  template: `
    <div class="h-screen w-screen bg-zinc-950 text-white flex flex-col overflow-hidden relative">
      ...
      <div class="absolute top-0 left-0 w-full bg-red-600/90 text-[10px] py-1 px-4 z-[100] font-mono flex justify-between">
      ...
      <header class="h-16 border-b border-zinc-800 bg-zinc-900/50 flex items-center justify-between px-6 z-20">
        <div class="flex flex-col">
          ...
          <span class="text-white text-sm font-bold italic uppercase">
             {{ store.users()[store.userId() || '']?.name || 'Anonymous' }}
          </span>
        </div>
      ...
      <main class="flex-1 relative flex items-center justify-center p-4 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black">
        ...
        @if (store.rngResult() === null) {
          ...
          <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md animate-in zoom-in duration-300">
            <app-result />
          </div>
        }
...
export class GameComponent  {
  ...
  // Access data from the global store
